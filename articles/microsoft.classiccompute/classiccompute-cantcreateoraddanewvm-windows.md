...
4. 如果要使用自定义映像来创建新 VM，请参阅以下文章，以验证你在创建映像时是否遵循了必要的步骤[Capture an image of an Azure Windows virtual machine created with the classic deployment model](https://docs.azure.cn/zh-cn/virtual-machines/windows/classic/capture-image/)（捕获使用经典部署模型创建的 Azure Windows 虚拟机的映像）
